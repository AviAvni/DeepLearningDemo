...
let evaluate (modelFile:string) (textSource:TextFormatSource) (device:DeviceDescriptor) =
    ...
    let mappings : TextFormat.InputMappings = {
        Features = [ { Variable = imageInput; SourceName = textSource.Features } ] 
...
let predict (modelFile:string) (textSource:TextFormatSource) (device:DeviceDescriptor) =
    let model = Function.Load(modelFile, device)
...
let predictOne (modelFile:string) (pixels:float32[]) (device:DeviceDescriptor) =
    let model = Function.Load(modelFile, device)
    ...
    let inputDataMap = 
        [
            imageInput, new Value(new NDArrayView(imageInput.Shape, pixels, device))
        ]
        |> dataMap
